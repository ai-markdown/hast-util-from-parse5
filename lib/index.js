...
function element(state, node) {
  ...
  // Switch content.
  // Only an HTML-namespace `<template>` has template contents: parse5 sets
  // `.content` per the HTML spec on `HTMLTemplateElement` alone, so inside
  // foreign content (`<svg>`, `<math>`) a `<template>` is an ordinary
  // foreign element whose children live in `childNodes` and whose
  // `.content` is `undefined` — reading it crashed the conversion.
  // Mirrors the namespace check `hast-util-to-html` uses when serializing.
  if (
    result.tagName === 'template' &&
    node.namespaceURI === webNamespaces.html
  ) {
